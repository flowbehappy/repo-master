...
function buildSystemPrompt(botName: string): string {
  ...
  return [
    ...
    "Answer based ONLY on the provided chat context, repo context, external knowledge context (if any), images (if any), and the user's question.",
    "Do NOT use any other knowledge sources or web browsing beyond the provided contexts.",
    ...
    "For TiDB/TiKV/PD/TiCDC/TiDB Cloud product facts: rely on external knowledge context when available; if it is missing or indicates lookup failure, do not guess—ask the user for version/environment details or to retry.",
